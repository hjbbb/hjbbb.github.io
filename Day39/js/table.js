...
function getData(judge) {
    ...
    selectedColors=[];
    return changeData(sourceData.filter(judge));
}

function changeData(data) {
    if(localData=JSON.parse(window.localStorage.getItem('data'))) {
        for(i in data) {
            for(j in localData) {
                if(data[i].region===localData[j].region&&data[i].product===localData[j].product) {
                    data[i].sale=localData[j].sale;
                }
            }
        }
    }
